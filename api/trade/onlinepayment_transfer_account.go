...
type V2TradeOnlinepaymentTransferAccountExtendInfo struct {
	...
	OrgRemittanceOrderId string `json:"org_remittance_order_id,omitempty"` //原汇款订单号	String	128	N	order_mode=B或order_mode=FD时，必填； 示例值：2022012614120615000
	DynamicFlag          string `json:"dynamic_flag,omitempty"`            //动态码标识	String	1	N	bank_mode=SNYH，page_flag=N 时生效 Y-动态，N-静态，默认：N
	TimeExpire           string `json:"time_expire,omitempty"`             //订单失效时间	String	14	N	格式：yyyyMMddHHmmss；示例值：20220925102245，若不填写：2个工作日自动关单
	FeeFlag              string `json:"fee_flag,omitempty"`                //手续费扣款标志	String	1	N	1: 外扣 2: 内扣 (默认取控台配置值)；示例值：1
}

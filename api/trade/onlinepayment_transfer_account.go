...
type V2TradeOnlinepaymentTransferAccountExtendInfo struct {
	...
	CertificateName      string //付款方名称	String	64	N	bank_mode=SNYH，且page_flag=N时必填；示例值：上海汇付支付有限公司
	BankCardNo           string //付款方银行卡号	String	2048	N	bank_mode=SNYH，且page_flag=N时必填；原文最大为19位，密文最大长度为2048；使用斗拱公钥做RSA加密； 示例值：b9LE5RccVVLChrHgo9lvp……PhWhjKrWg2NPfbe0mkQ== 支持的银行见清单；
	Remark               string //备注	String	1024	N	示例值：采购原料
	PageFlag             string //页面标识	String	1	N	Y-返回页面url，N-不返回页面url，默认：Y bank_mode=SNYH生效，示例值：Y
	OrderType            string //订单类型	String	1	N	P-支付，R-充值，默认：P；示例值：P
	NotifyUrl            string //异步通知地址	String	512	N	以'http'或者'https'开头， 示例值：http://www.huifu.com/getResp
	AcctMode             string //入账模式	String	8	N	bank_mode为银行模式时，必填
	DelayAcctFlag        string //延时标记	String	1	N	Y：延迟，银行大额支付仅支持延迟分账；示例值：Y，不传时则不分账 延时交易要调【交易确认】接口资金才能进入收款方账户，否则会停留在延时账户中。
	OrderMode            string //订单模式	String	2	N	O-订单下单模式，
	OrgRemittanceOrderId string //原汇款订单号	String	128	N	order_mode=B或order_mode=FD时，必填； 示例值：2022012614120615000
	DynamicFlag          string //动态码标识	String	1	N	bank_mode=SNYH，page_flag=N 时生效 Y-动态，N-静态，默认：N
	TimeExpire           string //订单失效时间	String	14	N	格式：yyyyMMddHHmmss；示例值：20220925102245，若不填写：2个工作日自动关单
	FeeFlag              string //手续费扣款标志	String	1	N	1: 外扣 2: 内扣 (默认取控台配置值)；示例值：1
}
